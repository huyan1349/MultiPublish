  ...
  try {
    ...
    if (!filled) {
      ...
      await chrome.storage.local.set({
        [`contentbridge_result_${PLATFORM}`]: { platform: PLATFORM, platformName: NAME, success: false, message: '未找到B站图文编辑器' },
      });
...
function _findAllFileInputs(): HTMLInputElement[] {
  ...
  document.querySelectorAll('*').forEach((el) => {
    ...
    if (sr) {
      sr.querySelectorAll<HTMLInputElement>('input[type="file"]').forEach((i) => inputs.push(i));
    }
  });
  return inputs;
}
