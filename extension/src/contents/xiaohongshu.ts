...
(async function init() {
  ...
  try {
    ...
    const filled = await tryFill(title, plainText, tags);
    if (!filled) {
      await chrome.storage.local.remove('contentbridge_fill');
      await chrome.storage.local.set({
        contentbridge_result: { platform: PLATFORM, platformName: NAME, success: false, message: '未找到小红书编辑器' },
      });
      showContentBridgeToast('ContentBridge 填充失败：未找到小红书编辑器', 'error');
      return;
    }
    ...
    await chrome.storage.local.remove('contentbridge_fill');
    const msg = err instanceof Error ? err.message : '小红书自动发布失败';
    await chrome.storage.local.set({
...
async function tryFill(title: string, body: string, tags: string[]): Promise<boolean> {
  const titleEl = await waitFor<HTMLInputElement>('input[placeholder*="标题"], #title', FILL_TIMEOUT);
  if (titleEl) setNativeValue(titleEl, title);

  const bodyEl = await waitFor<HTMLTextAreaElement>('textarea[placeholder*="笔记"], #content, textarea', FILL_TIMEOUT);
...
async function tryAutoPublish(): Promise<{ success: boolean; message: string }> {
  ...
  if (!publishBtn) return { success: false, message: '未找到小红书发布按钮' };
  ...
  for (let i = 0; i < 3; i++) {
    ...
    if (hasPublishSuccessSignal()) {
      return { success: true, message: '小红书笔记已自动提交发布' };
    }
  ...
  return hasPublishSuccessSignal()
    ? { success: true, message: '小红书笔记已自动提交发布' }
    : { success: true, message: '已自动点击小红书发布流程，请在页面确认最终状态' };
}

/* ── DOM Finders ── */

function findPublishButton(): HTMLElement | null {
  return findButtonByText(['发布', '发布笔记', '立即发布', '提交', '发表']);
}
...
function findConfirmButton(): HTMLElement | null {
  ...
  for (const root of dialogRoots) {
    const btn = findButtonByText(['确认', '确定', '确认发布', '发布', '提交'], root);
    if (btn) return btn;
  }

  return findButtonByText(['确认发布', '确认', '确定', '发布笔记']);
}
