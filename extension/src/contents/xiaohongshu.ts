...
(async function init() {
  ...
  try {
    ...
    if (bodyImages.length > 0) {
      ...
      if (bodyEl) {
        ...
        for (const img of bodyImages) {
          ...
          // 从 2000ms 降到 500ms：图片已插入 DOM，500ms 足够给富文本编辑器建索引
    ...
    // 从 1500ms 降到 600ms：一键排版通常 300-500ms 内完成，过度等待浪费时间
    await sleep(600);
    ...
    // 从 4000ms 降到 1800ms：下一步通常 1s 内切到发布页
    await sleep(1800);
...
async function clickAutoLayout(): Promise<boolean> {
  ...
  if (!btn) return false;
  clickElement(btn);
  // 从 1500ms 降到 400ms：DOM 变化由 MutationObserver 异步检测，固定等待可大幅缩短
  await sleep(400);
  return true;
}
...
async function clickNextStep(): Promise<boolean> {
  ...
  if (!btn) return false;
  clickElement(btn);
  // 从 1500ms 降到 400ms：同上
  await sleep(400);
  return true;
}

async function clickPublish(): Promise<boolean> {
  // 从 1500ms 降到 500ms：发布按钮出现通常 < 200ms
  await sleep(500);
  ...
  // 从 1500ms 降到 500ms：弹窗或成功信号通常 300ms 内出现
  await sleep(500);
...
async function _xhsUploadCover(img: { dataUrl: string; filename: string; mimeType: string }): Promise<boolean> {
  ...
  for (const inp of coverInputs) {
    ...
    // 从 2000ms 降到 600ms：图片上传走 XHR，处理完会触发 input 事件
    await sleep(600);
    // 早退：检测到封面预览已生成就立即返回，不再等后续 input 重试
    if (hasCoverPreview()) return true;
  }

  // Shadow DOM 兜底：只查 .creator-sidebar / 顶部 cover 区域附近的 custom elements，
  // 不再递归遍历整个文档（O(N) 太慢）
  const coverScopes = [
    '.creator-sidebar',
    '[class*="cover"]',
  ...
  for (const scope of coverScopes) {
    ...
    if (!rootEl) continue;
    const customs = Array.from(rootEl.querySelectorAll('*')).filter((el) => el.tagName.includes('-'));
    for (const el of customs) {
      ...
      for (const inp of shadowInputs) {
        if (editorContainer && editorContainer.contains(inp)) continue;
...
/** 检测封面预览是否已生成（图片上传成功的视觉信号） */
function hasCoverPreview(): boolean {
  // 封面区常见的预览类名
...
async function _xhsUploadBodyImage(img: { dataUrl: string; filename: string; mimeType: string }): Promise<boolean> {
  ...
  // 实时基准：每轮策略前重新检测，防止上轮成功但未及时返回时重复尝试
  const currentCount = () => bodyEl.querySelectorAll('img').length;
  const checkDone = () => currentCount() > before;
  ...
  if (editorContainer) {
    ...
    for (const inp of editorInputs) {
      _xhsSetInputFiles(inp, file);
      const ok = await _xhsWaitForImageInEditor(bodyEl, before, 3000);
      if (ok || checkDone()) { console.log('[XHS-IMG-BODY] S1 success'); return true; }
    }

    const allElements = editorContainer.querySelectorAll('*');
    for (const el of allElements) {
      const sr: ShadowRoot | null = (el as Element).shadowRoot;
      if (sr) {
        const shadowInputs = sr.querySelectorAll<HTMLInputElement>('input[type="file"]');
        for (const inp of shadowInputs) {
          _xhsSetInputFiles(inp, file);
          const ok = await _xhsWaitForImageInEditor(bodyEl, before, 3000);
          if (ok || checkDone()) { console.log('[XHS-IMG-BODY] S1 shadow success'); return true; }
        }
      }
    }
  }
  ...
  for (const inp of imageInputs) {
    ...
    _xhsSetInputFiles(inp, file);
    const ok = await _xhsWaitForImageInEditor(bodyEl, before, 3000);
    if (ok || checkDone()) { console.log('[XHS-IMG-BODY] S2 success'); return true; }
  }
  ...
  try {
    ...
    if (ok || checkDone()) { console.log('[XHS-IMG-BODY] S3 success'); return true; }
  ...
  if (capturedInput) {
    ...
    if (ok || checkDone()) { console.log('[XHS-IMG-BODY] S4 success'); return true; }
  }

  console.log('[XHS-IMG-BODY] all strategies failed');
  return checkDone(); // 最后兜底：即便所有策略"失败"，图片可能已插入
}
