...
在这个人人都是创作者的时代，好的想法不该被平台的门槛挡住。

一篇值得被看见的内容，从灵感到成文，从排版到分发——每一个环节都在消耗创作者最宝贵的东西：**表达的冲动**。当你在五个平台之间来回切换、反复调格式的时候，那个最初让你坐下来写作的火花，可能已经熄灭了。

MultiPublish 想做的，就是把这条路缩短到一趟旅程的距离。

**输入一个想法，AI 帮你展开。写一遍文字，五平台自动适配。点一次发布，内容真实抵达。**

我们不替代创作者——我们让创作回归创作本身。让每一个普通人，每一个有想法的人，都能用最低的成本、最流畅的体验，让自己的声音被更多人听到。

> *帮助每一个想法落地，让创作的边界不再由平台决定。*

[![License](https://img.shields.io/badge/license-MIT-blue.svg)](LICENSE)

**🎥 Demo 视频**：[B站 BV1fRVD6yEAq](https://www.bilibili.com/video/BV1fRVD6yEAq/)
...
- **图片上传（五平台）**：支持图片插入编辑器，自动转换为各平台格式 — 公众号工具栏上传、B站 shadow DOM 劫持、小红书多策略 DOM 注入、知乎剪贴板粘贴、微博数据直传
...
│   │       ├── adapters/    # 平台适配器（wechat/zhihu/bilibili/xiaohongshu/weibo）
...
MultiPublish 由两部分协同工作：

| 部分 | 定位 | 说明 |
|------|------|------|
| **Web 前端** (`web/frontend`) | 创作者工作台 | 灵感收集、AI 写作、格式适配、内容预览、发布管理——所有创作行为都在 Web 端完成 |
| **Chrome 扩展** (`extension`) | 真实发布引擎 | 接收 Web 端的发布指令，通过 Content Script 将内容真实注入各平台编辑器，走完完整发布流程 |

**这两者不是独立模块，而是一条流水线的上下游。** Web 端负责「把内容写好、适配好」，扩展负责「把内容真实送达」。两者通过 `web-bridge.ts` 通信，构成从灵感到分发的完整闭环。

如果你是评委或体验者，建议的完整流程是：

```
Web 前端 (localhost:5173) → 灵感/写作/适配 → 点击发布
    → Chrome 扩展自动打开平台页面 → 真实注入内容 → 自动发布
    → 评委打开目标平台页面，亲眼看到内容已填入、排版正确
```
...
```
┌─ Web 前端（编辑+预览）──────────────────────────────────┐
│  Tiptap 编辑器 → ParserService → 5 平台 Adapter → 预览   │
└──────────────────────┬──────────────────────────────────┘
                       │ chrome.runtime.sendMessage
                       ▼
┌─ Chrome 扩展（真实发布）─────────────────────────────────┐
│  Sidepanel（编辑面板）                                    │
│  Background SW（发布编排：并行+取消+storage信号）          │
│  Content Scripts × 5（DOM注入+自动点击发布按钮）           │
│   公众号: API 草稿+工具栏图片 │ 知乎: ClipboardEvent       │
│   B站: Tiptap+Shadow DOM    │ 小红书: 多策略DOM         │
│   微博: React Fiber 穿透     │                           │
└──────────────────────────────────────────────────────────┘
                       │
                       ▼
┌─ 五平台编辑器 ──────────────────────────────────────────┐
│  公众号后台 │ 知乎写文章 │ B站专栏 │ 小红书发布 │ 微博    │
│  ✅ 内容已填入，发布按钮已点击                              │
└──────────────────────────────────────────────────────────┘
```
...
七牛云 72 小时暑期实训营参赛作品，持续交付 130+ commits / 50+ PRs。

| 阶段 | 内容 |
|------|------|
| 基础搭建 | 项目规划、前后端脚手架、数据库、4 平台 Adapter + ParserService |
| 前端页面 | Editor + Preview + Records + Dashboard + 单平台发布 |
| Tiptap 编辑器 | 所见即所得编辑 + Demo 填充 + 统计面板 |
| Chrome 扩展 | Plasmo 框架 + Content Script DOM 注入 + Sidepanel UI |
| 真发布链路 | 公众号/知乎/B站/小红书 DOM 注入 + 自动点击发布 |
| AI 集成 | DeepSeek 写作助手 + 流式动画 + 大纲展开 |
| 图片系统 | 五平台多策略图片上传 + 拖拽缩放编辑器 |
| UI 重设计 | 暗夜编辑室主题 + 像素极简 + 品牌色 CSS 变量 |
| 批量发布 | 并行一键全发 + 取消发布 + 独立 storage key |
| 平台扩展 | 微博平台完整支持 + B站 Tiptap 适配 + 公众号 API 草稿 |
