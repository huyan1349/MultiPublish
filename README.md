...
- **Windows 流畅度优化**：自动识别 Windows 环境，降低 fixed 背景、backdrop blur、无限装饰动画和页面入场动画的渲染开销
- **一键 Demo 填充**：自动填入示例，快速体验完整流程
- **可扩展适配器架构**：新增平台只需实现 `PlatformAdapter` 接口并注册

## 近期稳定性更新

- **稿件删除可靠性**：删除稿件时会同步清理平台适配输出和发布记录，避免数据库外键关联导致“点击删除但实际未删除”。
- **历史稿件预览兜底**：从稿件列表或 Dashboard 打开历史稿件时，如果尚未生成平台输出，预览页会自动补生成五个平台版本再展示。
- **Windows 性能保护**：前端运行时会给 Windows 浏览器启用 `windows-render-safe` 和 Framer Motion reduced-motion 配置，减少滚动、列表切换和编辑器流式动画中的掉帧。
...
| 稳定性优化 | 稿件级联删除 + 历史稿件预览兜底 + Windows 流畅度优化 |
