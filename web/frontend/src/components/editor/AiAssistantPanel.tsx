...
import { X, Sparkles, RefreshCw, Check, Copy, Wand2, Zap, ChevronDown, ChevronUp, ArrowRight, AlertCircle, Settings } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { optimizeContent, optimizeSelection } from '../../services/deepseek';
import { useAiConfig } from '../../hooks/useAiConfig';
import ConnectionStatusBadge from '../ai/ConnectionStatusBadge';
...
export default function AiAssistantPanel({
  ...
  // 订阅 store 变化，确保 banner 状态总是反映当前最新配置
  // （之前用 useState 在 open 时快照，导致保存配置后 banner 不更新）
  // 用 selector 精确订阅，避免无谓的全 store 订阅
  const providerId = useAiConfig((s) => s.providerId);
  const model = useAiConfig((s) => s.model);
  const apiKey = useAiConfig((s) => s.apiKey);
  const showConfigBanner = !(providerId && model && apiKey);
        ...
        >
          <div className="flex h-full w-[400px] flex-col">
            ...
            <div className="flex-1 overflow-y-auto px-4 py-5 space-y-5 scrollbar-thin">
              {/* 未配置警告 */}
              {showConfigBanner && (
                <div className="rounded-[16px] border border-amber-200 bg-amber-50/80 p-3.5 flex items-start gap-2.5">
                  <AlertCircle size={14} className="text-amber-600 mt-0.5 shrink-0" />
                  <div className="flex-1 min-w-0">
                    <div className="text-[12px] text-amber-900 font-medium">尚未配置 AI 供应商</div>
                    <div className="text-[11px] text-amber-700 mt-1 leading-relaxed">
                      请先选择供应商（DeepSeek / OpenAI / Claude / Kimi）并填入 API Key。
                    </div>
                  </div>
                  <button
                    onClick={() => setSettingsOpen(true)}
                    className="shrink-0 inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-amber-600 text-white text-[11px] hover:bg-amber-700 transition"
                  >
                    <Settings size={10} />
                    去配置
                  </button>
