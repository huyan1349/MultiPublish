 ...
 * AI 内容生成服务。
 *
 * 历史：本文件早期直连 DeepSeek（hardcoded model = 'deepseek-chat'）。
 * 现在统一走 aiClient（不绑定供应商），由用户在设置页选择 provider/model/apiKey。
 *
 * 业务侧的 11 个 prompt 函数（optimizeContent、generateTitle、suggestTags…）保持签名稳定，
 * 调用方（Editor/Inspiration/PlatformCard/AiAssistantPanel）无需改动。
 */

import { chat, streamChat, type ChatMessage, type StreamCallback } from './aiClient';

// 重新导出 aiClient 的核心类型供旧引用使用
export type { ChatMessage, StreamCallback };
...
回答要具体、有实操性，不要空泛的建议。每次回复聚焦1-3个要点。`,
  ...
  return chat(allMessages, { temperature: 0.7 });
}

// ---------------------------------------------------------------------------
// 大纲生成完整内容（含 4 平台 × 4 格式 Prompt）
// ---------------------------------------------------------------------------
