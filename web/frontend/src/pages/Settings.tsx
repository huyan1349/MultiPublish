...
import { Trash2, Check, AlertCircle, Zap, ExternalLink, ArrowRight, Monitor, Sparkles } from 'lucide-react';
      ...
      <div className="mx-auto flex max-w-[1100px] flex-col gap-6">
        ...
        <section className="grid gap-6 xl:grid-cols-[1fr_360px]">
          <div className="space-y-6">
            ...
            <div className="px-card px-paper p-6">
              <div className="px-label mb-5">AI 供应商</div>
              <div className="rounded-[24px] border border-[rgba(49,56,45,0.1)] bg-[rgba(255,255,255,0.72)] p-5">
                <div className="flex items-center justify-between gap-4 mb-3">
                  <div>
                    <div className="text-[14px] text-[var(--ink)] flex items-center gap-2">
                    ...
                    {aiConfig.providerId && aiConfig.model && aiConfig.apiKey ? (
                      <p className="mt-2 text-[13px] leading-6 text-[var(--ink-soft)]">
                        {getProvider(aiConfig.providerId)?.label} · <span className="font-mono">{aiConfig.model}</span>
                      </p>
                    ) : (
                      <p className="mt-2 text-[13px] leading-6 text-[var(--ink-soft)]">
                        {aiConfig.providerId && aiConfig.model && !aiConfig.apiKey
                          ? '已选择供应商和模型，但未填入 API Key。'
                          : '尚未配置。点击"管理"选择供应商、模型并填入 API Key。'}
                      </p>
                    )}
                  </div>
                  {aiConfig.providerId && aiConfig.model && aiConfig.apiKey ? (
                    <span className="px-tag">
                      <Check size={10} className="text-green-600" /> 已配置
                    </span>
                  ) : (
                    <span className="px-tag">
                      <AlertCircle size={10} className="text-amber-500" /> 未配置
                    </span>
                  )}
                </div>
                <button
                  onClick={() => setAiModalOpen(true)}
                  className="mt-3 w-full flex items-center justify-center gap-2 rounded-[14px] border border-[var(--accent)]/25 bg-[var(--accent)]/8 px-4 py-2.5 text-[13px] text-[var(--accent-deep)] hover:bg-[var(--accent)]/14 transition-all"
                >
                  管理 AI 供应商
                  <ArrowRight size={12} />
                </button>
